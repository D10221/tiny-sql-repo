import client, {
  ...
  update,
  FetchLike,
} from "../src/action-handler-client";

const respondWith = (answer: { ok: boolean; jsonBody: any }) => {
...
describe("tny repo remote client", () => {
  ...
  it("invoke count", async () => {
    const { requestBody, result } = await test(
      count,
      "",
      respondWith({ ok: true, jsonBody: 1 }),
    );
  ...
  it("invoke exists", async () => {
    const { requestBody, result } = await test(
      exists,
      "",
      respondWith({ ok: true, jsonBody: true }),
    );
    expect(requestBody).toMatchObject({
      payload: { filter: "" },
      type: "exists",
  ...
  it("invoke find", async () => {
    const { requestBody, result } = await test(
      find,
      { filter: "", columns: [] as any, take: 1, skip: 1, desc: true },
      respondWith({ ok: true, jsonBody: [] }),
    );
    expect(requestBody).toMatchObject({
      payload: { filter: "", columns: [] as any, take: 1, skip: 1, desc: true },
      type: "find",
    });
  ...
  it("invoke get", async () => {
    const { requestBody, result } = await test(
      get,
      { columns: [] as any, id: 1 },
      respondWith({ ok: true, jsonBody: { id: 1 } }),
    );
    expect(requestBody).toMatchObject({
      payload: { columns: [] as any, id: 1 },
      type: "get",
    });
  ...
  it("invoke remove", async () => {
    const { requestBody, result } = await test(
      remove,
      { id: "x" as any } as any,
      respondWith({ ok: true, jsonBody: "ok" }),
    );
  ...
  const fetchMock = (
    ok: boolean,
    answer: (url?: RequestInfo, request?: RequestInit) => any,
  ): FetchLike => (url: RequestInfo, request?: RequestInit) => {
  ...
  it("Builds client", () => {
    (global as any).fetch = fetchMock(false, () => {
      throw new Error("It Fetch Should not be invoked");
    });
    const {
      count,
      exists,
